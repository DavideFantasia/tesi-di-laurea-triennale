...
void main(){
  vec2 uv = (TexCoords - 0.5 * uResolution) / uResolution.y; // Centers the UV coordinates on the center of the canvas
  vec3 col = vec3 (0); // Color to be drawn on screen

  // Bending the world
  int recursionCount = -1 + int (mod (uTime, 14.0) * 0.5); // Number of iterations for the fractal
  float d = kochsCurve (uv, recursionCount); // Distance from the fractal

  // Drawing the fractal
  float lineSmoothness = 4.0 / uResolution.y; // Smoothness / thickness of the line (higher number = smoother / thicker, lower numbers = sharper / thinner)
  col.r += smoothstep (lineSmoothness, 0.0, d) * 0.5;

  // Drawing the axis
  lineSmoothness = 1.0 / uResolution.y; // Descreases line smoothness for sharper / thinner lines
